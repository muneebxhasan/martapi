...
def login_access_token(db:DB_session,
    ...
    token_data = Token(
        access_token=security.create_access_token( subject={"sub":user.id, "scopes": scopes}, expires_delta=access_token_expire), # access token with id
        refresh_token=security.create_access_token(subject={"sub":user.email, "scopes": scopes}, expires_delta=refresh_token_expire), # refresh token with email
